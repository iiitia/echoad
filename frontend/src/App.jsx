...
const MAX_RETRY_DELAY = 30000  // cap at 30 s
...
function useWebSocket(url, onMessage) {
  ...
  // Keep ref in sync so connect() closure never goes stale
  ...
  const scheduleRetry = useCallback(() => {
    ...
    console.log(`🔁 Reconnecting in ${(delay / 1000).toFixed(1)}s (attempt ${retryCount.current})…`)
    retryRef.current = setTimeout(() => connect(), delay)   // eslint-disable-line no-use-before-define
  }, [])                                                    // connect defined below via ref trick
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      ws.onopen = () => {
        ...
        // Keepalive ping every 25 s (Render closes idle WS after 30 s)
      ...
      ws.onclose = (e) => {
        ...
        // Auto-reconnect unless the closure was clean/intentional (code 1000)
      ...
      <div style={{ minHeight: '100vh', padding: '18px 20px', maxWidth: 1180, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18, flexWrap: 'wrap', gap: 10 }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <span className={`pill ${isLive ? 'pill-live' : status === 'connecting' ? 'pill-pipeline' : 'pill-offline'}`}>
              <span style={{ width: 7, height: 7, borderRadius: '50%', background: isLive ? '#00e676' : status === 'connecting' ? '#00b0ff' : '#455a64', display: 'inline-block' }} className={isLive || status === 'connecting' ? 'blink' : ''} />
              {isLive ? 'LIVE STREAMING' : status === 'connecting' ? 'CONNECTING…' : status.toUpperCase()}
            </span>
